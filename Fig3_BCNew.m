...
for test = 1:3
    Tmax = 300; ts = 0.1; dt = 0.001; N = 10; NN = round(Tmax/dt);
    Sav = zeros(round(Tmax/ts),3);
    P_I = ones(NN,1)*1.1551;       M = ones(NN,1);
    ...
    %%%%%% CHANGE IF MDMX = 0 OR MDMX = 1
    X = 0;
    %%%%%% MAYBE CHANGE lambda2 = 0
    ...
    while (RT < Tmax-dt)
        ...
        if (RT > Timp)
            ...
            k2 = k20*(1 + lambda1*X) * (1 - ATR/(rho1 + ATR));           
