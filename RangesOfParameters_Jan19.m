...
for test = 1:3
    ...
    P_I = ones(NN,1)*0.5*test;       M = ones(NN,1)*0.15;
...
fpm = k7/k8*fpp;
Delta = k2*k4*k8*fpm/(k4+fpp)^2 + k2*k7*fpp/(k4+fpp);
Tau = -(k2*k4*fpm/(k4+fpp)^2 + k8);
omega = 0.5*sqrt(4*Delta-Tau^2);
...
P = [];
T = [];
for i = 2:length(X1)-1;if X1(i-1) < X1(i) && X1(i) > X1(i+1)
        T = [T;i*ts]; end; end
    

1/omega*2*pi
TNY = mean(T(2:end)-T(1:end-1))


RP = 4.9;
4.9/(mean(T(2:end)-T(1:end-1)));
k1n = k1/(4.9/(mean(T(2:end)-T(1:end-1))));
k2n = k2/(4.9/(mean(T(2:end)-T(1:end-1))));
k7n = k7/(4.9/(mean(T(2:end)-T(1:end-1))));
k8n = k8/(4.9/(mean(T(2:end)-T(1:end-1))));
TDeln = 4.9/mean(T(2:end)-T(1:end-1))*(dt*TDel1);
Pars = [k1n k2n k7n k8n TDeln]